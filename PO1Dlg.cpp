...
void CPODlg::OnBnClickedButtonLoad()
{
	...
	{
		m_imgIN.dib.LoadDIB(FileDlg.GetPathName());
		m_imgOUT.dib.LoadDIB(FileDlg.GetPathName());
		InvalidateRect(NULL);
	}
	...
	if (sOption == L"convert to greyscale")
	{
		...
		if (m_imgIN.dib.bitmapInfoHeader->biBitCount == 1) {
			for (int i = 0; i < m_imgIN.dib.bitmapInfoHeader->biWidth * m_imgIN.dib.bitmapInfoHeader->biHeight; i++) {
				...
				int y = i / m_imgIN.dib.bitmapInfoHeader->biWidth;

				BYTE grayscale;
				bool pixel = m_imgIN.dib.GetPixel1(x, y);
				if (pixel){
					grayscale = 0x00;
				}
				else {
				...
				m_imgOUT.dib.SetPixel8(x, y, grayscale);
			}
		}
		else if (m_imgIN.dib.bitmapInfoHeader->biBitCount == 8) {
			for (int i = 0; i < m_imgIN.dib.bitmapInfoHeader->biWidth * m_imgIN.dib.bitmapInfoHeader->biHeight; i++) {
				int x = i % m_imgIN.dib.bitmapInfoHeader->biWidth;
				int y = i / m_imgIN.dib.bitmapInfoHeader->biWidth;

				BYTE pixel = m_imgIN.dib.GetPixel8(x, y);

				BYTE grayscale = pixel;
				m_imgOUT.dib.SetPixel8(x, y, grayscale);
			}
		}
		else if (m_imgIN.dib.bitmapInfoHeader->biBitCount == 24) {
			for (int i = 0; i < m_imgIN.dib.bitmapInfoHeader->biWidth * m_imgIN.dib.bitmapInfoHeader->biHeight; i++) {
				int x = i % m_imgIN.dib.bitmapInfoHeader->biWidth;
				int y = i / m_imgIN.dib.bitmapInfoHeader->biWidth;

				RGBTRIPLE pixel = m_imgIN.dib.GetPixel24(x, y);

				BYTE grayscale = 0.2989 * pixel.rgbtRed + 0.5870 * pixel.rgbtGreen + 0.1140 * pixel.rgbtBlue;
				m_imgOUT.dib.SetPixel8(x, y, grayscale);
			}
		}
	}
...
void CPODlg::OnBnClickedButtonSave()
{
	...
	if (FileDlg.DoModal() == IDOK)
	{
		m_imgOUT.dib.SaveDIB(FileDlg.GetPathName());
	}
}
